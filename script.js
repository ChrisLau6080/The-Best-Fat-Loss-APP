...
// NEW CHART ELEMENT REFERENCE
const macroChartCanvas = document.getElementById('macroPieChart'); 
...
/**
 * Renders the macro nutrient pie chart based on calorie allocation.
 * @param {number} calCarbs - Calories allocated to Carbs.
 * @param {number} calProtein - Calories allocated to Protein.
 * @param {number} calFat - Calories allocated to Fat.
 */
function renderMacroPieChart(calCarbs, calProtein, calFat) {
    if (macroChartInstance) {
        macroChartInstance.destroy(); // Destroy previous chart instance if it exists
    }

    const ctx = macroChartCanvas.getContext('2d');
    ...
    macroChartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Calories',
                data: [calCarbs, calProtein, calFat],
                backgroundColor: [
                    'rgb(54, 162, 235)', // Blue for Carbs
                    'rgb(255, 99, 132)', // Red for Protein
                    'rgb(255, 205, 86)'  // Yellow for Fat
                ],
                hoverOffset: 4
            }]
        },
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            if (context.parsed !== null) {
                                label += new Intl.NumberFormat('en-US', { style: 'decimal' }).format(context.parsed) + ' cal';
                            }
                            return label;
                        }
                    }
...
/**
 * Initializes the main app page by loading data from Local Storage.
 */
function initializeApp() {
    ...
    // Attempt to render chart if goal is set
    if (dailyGoal > 0) {
        const calCarbs = dailyGoal * 0.50;
        const calProtein = dailyGoal * 0.25;
        const calFat = dailyGoal * 0.25;
        renderMacroPieChart(calCarbs, calProtein, calFat);
    }
}
...
/**
 * Handles the calculation and saving of the daily calorie goal (Maintenance Goal).
 */
function handleCalorieGoal(e) {
    ...
    // Store the weekly loss rate for the progress page to use for its check
    localStorage.setItem('lastWeeklyLossRate', weeklyLossRate);
